...
function listarTimezones(){
    try {
        fetch(`https://api.ipgeolocation.io/timezone?apiKey=d4832a3a1d4c47dfaf6ad1eda8daba03&tz=${listaDeTimezones.value}`)
            ...
            .then(timezone => {
                pararContagemTempo(); // Parar a contagem de tempo anterior, se houver

                hora.innerHTML = timezone.time_24;
                pais.innerHTML = timezone.timezone.split('/')[1];
                let horaAgora = Number(timezone.time_24.split(':')[0]);

                if(horaAgora >= 5 && horaAgora < 13){
                    document.querySelector('body').style.backgroundImage = 'url("img/amanhecer.jpg")';
                    container.classList.remove('light');
                    icone.setAttribute('name', 'sun');
                    frase.innerHTML = "Acorda minino";
                } else if(horaAgora >= 12 && horaAgora < 18){
                    document.querySelector('body').style.backgroundImage = 'url("img/entardecer.jpg")';
                    container.classList.add('light');
                    icone.setAttribute('name', 'sun');
                    frase.innerHTML = "Bora tomar um café";
                } else {
                    document.querySelector('body').style.backgroundImage = 'url("img/anoitecer.jpg")';
                    container.classList.add('light');
                    icone.setAttribute('name', 'moon');
                    frase.innerHTML = "Dorme minino";
                }

                atualizarHora(); // Iniciar a contagem de tempo em tempo real
            });
    } catch(error){
        alert("Deu ruim");
    }
...
function atualizarHora(){
    clearInterval(intervalId); // Certificar-se de que a contagem anterior seja interrompida
    ...
    intervalId = setInterval(() => {
        ...
        hora.innerHTML = `${t_hora.toString().padStart(2, '0')}:${t_minuto.toString().padStart(2, '0')}:${t_segundo.toString().padStart(2, '0')}`;
    }, 1000);
}
